...
import com.bayusurapati.ukl_food.PlaceYourOrderActivity
...
class PlaceYourOrderAdapter(val menuList: List<Menus?>?): RecyclerView.Adapter<PlaceYourOrderAdapter.MyViewHolder>() {
    ...
    inner class MyViewHolder(view: View): RecyclerView.ViewHolder(view){
        ...
        val menuQuantity: TextView = view.findViewById(R.id.menuQuantity)


        fun bind(menu:Menus){
            menuName.text = menu?.name!!
            menuPrice.text = "Price Rp."+String.format("%.2f", menu?.price!! * menu.totalInCart)
            menuQuantity.text = "Quantity:"+menu?.totalInCart.toString()
